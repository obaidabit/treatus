  ...
  fetch(`diseases/patient/info?patientId=${patient_id}`)
    ...
    .then((diseases) => {
      diseases.forEach((element) => {
        ...
        patdiseases.className = "col-sm-12 col-lg-3 mb-3";
...
function getPotions(mid, medname) {
  ...
  fetch(`potions/medicine?patientId=${patient_id}&medicineId=${mid}`)
    .then((res) => res.json())
    .then((potions) => {
  ...
  fetch(`medicines/patient/info?patientId=${patient_id}`)
    ...
    .then((medicines) => {
      medicines.forEach((element) => {
        ...
        patmedicines.className = "col-sm-12 col-lg-3 mb-3";
...
fetch(`appointments/info?appointmentId=${url.get("appid")}`)
  ...
  .then((data) => {
    ...
    document.getElementById(
      "viewpatientreport"
    ).href = `/report?patientId=${data.patient_id}`;
    fetch(`patients/info?patientId=${data.patient_id}`)
      .then((res) => res.json())
      .then((patientdata) => {
    ...
    getDiseases(data.patient_id);
    getMedicines(data.patient_id);
  });
...
document.getElementById("saveviewapp").addEventListener("click", () => {
